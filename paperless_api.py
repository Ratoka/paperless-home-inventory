...
class PaperlessClient:
    ...
    def _do_resolve_task(self, task_id: str, timeout: int = 90) -> int | None:
        """
        Poll the Paperless task API until the task succeeds, fails, or times out.
        Returns the related document ID on success.
        Raises RuntimeError on FAILURE/REVOKED with the Paperless error message.
        Returns None only on genuine timeout.
        """
            ...
            while time.monotonic() < deadline:
                ...
                if tasks:
                    ...
                    if status in ("FAILURE", "REVOKED"):
                        detail = task.get("result") or status
                        # Paperless reports duplicates as FAILURE — extract the
                        # existing document ID and treat it as a successful upload.
                        dup = re.search(r"duplicate of .+?\(#(\d+)\)", detail or "")
                        if dup:
                            existing_id = int(dup.group(1))
                            logger.info(
                                "Paperless task %s: duplicate of document #%d — "
                                "using existing document",
                                task_id, existing_id,
                            )
                            return existing_id
                        logger.error(
                            "Paperless task %s ended with status %s: %s",
                            task_id, status, detail,
                        )
                        raise RuntimeError(detail)
                time.sleep(3)
        logger.warning("Paperless task %s did not complete within %ds", task_id, timeout)
        return None
